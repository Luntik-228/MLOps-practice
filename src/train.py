...
def set_champion_alias(model_name):
    client = MlflowClient()
    versions = client.search_model_versions(f"name='{model_name}'")

    if not versions:
        print("Версии модели не найдены, alias не назначен.")
        return
    ...
    print(
        f"Alias champion назначен модели {model_name}, version {latest_version.version}"
    )
...
def train_model():
    ...
    set_champion_alias("Iris_RF_Model")
